...
(function (global) {
  ...
  var standalone = false;       // true if we could not borrow Sfx's context
  ...
  function build() {
    ...
    var shared = global.Sfx && global.Sfx.getContext && global.Sfx.getContext();
    if (!shared) {
      // Not fatal - music still has to play - but it means we are running on a
      // second AudioContext, which on mobile can get suspended independently
      // and die without anything visibly breaking. Callers can assert on
      // isStandaloneContext() rather than discovering this in the field.
      standalone = true;
    }
    ctx = shared || new Ctor();
  ...
  var Music = {
    ...
    /**
     * The context the music is actually playing through.
     *
     * Exposed so callers can assert identity with Sfx.getContext(). Two
     * contexts is the same class of bug as a master gain wired as a sibling
     * input: everything looks fine from the outside and the audio is wrong.
     */
    context: function () { return ctx; },
    isStandaloneContext: function () { return standalone; },

    /**
     * The one AudioContext this app is allowed to have.
     *
     * Exposed so callers can assert identity with Sfx.getContext(). Two
     * contexts means one of them gets suspended on mobile and its audio dies
     * silently - the same class of bug as a master gain wired as a sibling
     * input, which is invisible from the outside.
     */
    context: function () { return ctx; },
