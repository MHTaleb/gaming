...
function hardenMainActivity() {
  ...
  for (const file of found) {
    ...
    if (src.includes('neonstack-hardening')) continue;

    const isKotlin = file.endsWith('.kt');
    const guard = isKotlin
      ? `
        // neonstack-hardening: never expose the WebView to remote debugging in release.
...
`;
    ...
    if (src.includes('neonstack-hardening')) write(file, src, path.basename(file));
  }
}
...
function maybeMinify() {
  ...
  if (gradle.includes('neonstack-minify')) return;
