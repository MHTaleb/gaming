...
Read `reviews/README.md` and `reviews/RV-001/request.json`, then `reports/validation-001.md`, and execute this correction batch before continuing RZ-006. Current structured review records take precedence over historical instructions. The reviewed implementation is `caf1cce1b7c57b6e88a9b65c52e6000ba25d736f`. The report records validator findings; the previous backlog statuses remain the original agent claims and are not independent approval.
...
2. Read AGENTS.md, the validation report and relevant specs. Keep one writer. Use four correction items V-001 through V-004 in your response; record their progress in reviews/RV-001/response.json; only create linked RZ child tickets if a finding needs decomposition. Do not delete original evidence or silently call a validator finding resolved.
...
7. Submit `reviews/RV-001/response.json` with each V-ID, implementation commit, changed paths and hashed command evidence under reviews/README.md. An optional reports/validation-001-response.md may explain the results, but response.json is authoritative for submission status. Create a normal implementation PR if none exists. Do not claim validator acceptance on the validator's behalf.
...
- Hardware identity is already confirmed by Housseyn as RTX 4050 / 6 GB; laptop execution claims have accessible evidence.
