...
(function () {
  ...
  function openDetail(id) {
    ...
    meta.appendChild(badge('size', it.size));
    ...
    ['steps', 'outputs', 'checks'].forEach(function (key) {
      if (!it[key] || !it[key].length) return;
      var list = el(key === 'steps' ? 'ol' : 'ul');
      it[key].forEach(function (entry) { list.appendChild(el('li', null, entry)); });
      body.appendChild(section(body, key, list));
    });
    if (it.phase) body.appendChild(section(body, 'phase', para(it.phase)));
    if (it.owner) body.appendChild(section(body, 'owner', para(it.owner)));
    if (it.evidence && it.evidence.length) {
      var evidence = el('ul');
      it.evidence.forEach(function (entry) {
        evidence.appendChild(el('li', null, entry.command + ' — ' + entry.result + ' — ' + entry.artifact + ' — ' + entry.environment));
      });
      body.appendChild(section(body, 'evidence', evidence));
    }
