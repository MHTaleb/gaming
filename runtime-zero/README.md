...
**Current deliverable: the installed, verified local toolchain with a project-local launcher (`source tools/env.sh` — Node 22+, uv, managed Python, pinned Godot 4.7.2), the working review inbox and Kanban board, a launchable Godot project (`game/`), the deterministic combat core (`game/tests/run_tests.gd`), validated content packs (`game/tests/content_tests.gd`), the playable three-encounter campaign (title → loadout → connected encounters → result screen; `game/tests/run_flow_tests.gd`), local progress persistence with backup recovery (`game/tests/save_tests.gd`), and an accessibility/feedback pass with placeholder sound cues, mute, reduced motion and keyboard/touch polish (`game/tests/feedback_tests.gd`, `reports/prototype-accessibility.md`). Replay fixtures pin the exact combat behavior — outcomes, event digests and final state hashes — and a root CI workflow runs the whole headless battery on the pinned engine (`game/tests/replay_tests.gd`, `tools/make_replay_fixtures.gd`, `.github/workflows/runtime-zero-game.yml`).** The owner playtest review (RZ-012) and the real generated art and audio phases (RZ-016+, RZ-023+) are the remaining gates.** Current code status and review records live in `reviews/`; see `reports/prototype-log.md` and `reports/environment.md` for executed evidence.
