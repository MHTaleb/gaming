...
**Current deliverable: the installed, verified local toolchain with a project-local launcher (`source tools/env.sh` — Node 22+, uv, managed Python, pinned Godot 4.7.2), the working review inbox and Kanban board, a launchable Godot project (`game/`), the deterministic combat core behind `game/tests/run_tests.gd`, validated content packs (`game/content/`, checked by `game/src/infrastructure/content_repository.gd` and `game/tests/content_tests.gd`), and the playable combat scene (`game/scenes/combat.tscn` — one encounter, Attack/Guard/Skill, intent, combat log, win/lose/retry, keyboard+mouse+touch) behind `game/tests/presentation_tests.gd`. Loadout/run loop and encounter progression, saves, the headless simulator, the local asset studio, audio and Android are still pending.** Current code status and review records live in `reviews/`; see `reports/prototype-log.md` and `reports/environment.md` for executed evidence. This is a single-encounter playable prototype, not yet the full three-encounter slice.
