...
**Current deliverable: the installed, verified local toolchain with a project-local launcher (`source tools/env.sh` — Node 22+, uv, managed Python, pinned Godot 4.7.2), the working review inbox and Kanban board, a launchable Godot project (`game/`), the deterministic combat core (`game/tests/run_tests.gd`), validated content packs (`game/tests/content_tests.gd`), the playable three-encounter campaign (title → loadout → connected encounters → result screen; `game/tests/run_flow_tests.gd`), and local progress persistence with atomic writes, backup recovery and migration (`game/tests/save_tests.gd`).** Accessibility polish, replay fixtures/CI, real generated art and audio are still pending (RZ-010+).** Current code status and review records live in `reviews/`; see `reports/prototype-log.md` and `reports/environment.md` for executed evidence.
