...
**Current deliverable: the installed, verified local toolchain with a project-local launcher (`source tools/env.sh` — Node 22+, uv, managed Python, pinned Godot 4.7.2), the working review inbox and Kanban board, a launchable Godot project (`game/`), the deterministic combat core (`game/tests/run_tests.gd`), validated content packs (`game/tests/content_tests.gd`), and the playable three-encounter campaign: title with procedural key art, equipment loadout, three connected encounters with rewards, win/lose/retry and a result screen — all behind `game/tests/run_flow_tests.gd` and `game/tests/presentation_tests.gd`.** Saves/persistence, accessibility polish, replay fixtures/CI, real generated art and audio are still pending (RZ-009+).** Current code status and review records live in `reviews/`; see `reports/prototype-log.md` and `reports/environment.md` for executed evidence.
