...
3. Read backlog/backlog.json. Run `node tools/backlog.js --ready` when Node exists. Without Node, read the JSON directly and execute the Node bootstrap in docs/SETUP.md first.
4. Resume an owned `doing` ticket, or choose the highest-ranked `next` ticket. Every dependency must be `done`. When none is ready, promote the earliest-phase `later` ticket whose dependencies are done and whose decision gates are satisfied. Run validation after promotion. Do not activate optional expansion work without its gate.
