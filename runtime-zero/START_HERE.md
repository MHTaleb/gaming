...
1. Establish whether the terminal is on Housseyn's Lenovo, WSL on that Lenovo, a container, or a remote machine. Record the distinction. His reported GPU is an RTX 4060 laptop GPU; actual VRAM, RAM and SSD availability are still unmeasured.
...
5. Run `node tools/backlog.js --ready`; claim one eligible ticket. Install needed tooling as part of each phase. A dependency is complete only if the recorded evidence supports it.
