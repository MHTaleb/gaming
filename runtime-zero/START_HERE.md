...
Available now: backlog validator, ready query, review inbox (`node tools/reviews.js`), read-only board server, specification verification, specification documents, a safe read-only machine doctor, the project toolchain launcher (`tools/env.sh` — Node 22+, uv, pinned Godot 4.7.2), the launchable Godot project (`game/project.godot` with the title scene), the deterministic combat core with its headless test entry point (`game/tests/run_tests.gd`), validated content packs (`game/content/`) with the validating repository and tests (`game/src/infrastructure/content_repository.gd`, `game/tests/content_tests.gd`), and the playable combat scene (`game/scenes/combat.tscn`, entered from the title with Enter/click; headless suite `game/tests/presentation_tests.gd`; desktop smoke `-- --combat-smoke <dir>`).

Future contracts, NOT executable yet: loadout/result scenes, the run loop and encounter progression, saves, the headless simulation and balance tooling, Studio MCP Python package, ComfyUI workflows, asset promotion commands, audio backends and Android exports. Their implementing tickets must create them before invoking them. Do not mistake a command shown in a specification for an installed tool.
