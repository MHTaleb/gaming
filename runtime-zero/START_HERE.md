...
Available now: backlog validator, ready query, review inbox (`node tools/reviews.js`), read-only board server, specification verification, specification documents, a safe read-only machine doctor, the project toolchain launcher (`tools/env.sh` — Node 22+, uv, pinned Godot 4.7.2), the launchable Godot project (`game/project.godot` with the provisional title scene), the deterministic combat core with its headless test entry point (`game/tests/run_tests.gd`), and validated content packs (`game/content/`) with the validating repository and tests (`game/src/infrastructure/content_repository.gd`, `game/tests/content_tests.gd`).

Future contracts, NOT executable yet: combat UI scenes, loadout/result scenes, the run loop and saves, the headless simulation and balance tooling, Studio MCP Python package, ComfyUI workflows, asset promotion commands, audio backends and Android exports. Their implementing tickets must create them before invoking them. Do not mistake a command shown in a specification for an installed tool.
